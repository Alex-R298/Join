...
async function fetchContacts(path) {
  ...
  const contactsListDetails = document.getElementById(
...
function editContact(contactId, name, email, phone, initials, avatarColor) {
  ...
  if (editOverlay) {
    editOverlay.innerHTML = editContactTemplate(
      contactId,
      name,
      email,
      phone,
...
document.addEventListener("click", function (event) {
  ...
  if (headline?.contains(event.target) && btn && !btn.contains(event.target)) {
    if (
      menu &&
      !menu.classList.contains("d_none") &&
      menu.style.display !== "none"
    ) {
      closeDropdown();
    }
...
=======
>>>>>>> 37bdbd78a8e12bb584878673381a2f0b0cc652ef
